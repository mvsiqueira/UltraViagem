...
FOLDER = r'G:\Meu Drive\Viagens\2019-10 Serra Ga' + chr(250) + 'cha'
XLSX   = FOLDER + r'\Viagem Serra Ga' + chr(250) + 'cha.xlsx'
PFX    = 'serragaucha-2019'

wb      = openpyxl.load_workbook(XLSX)
wb_data = openpyxl.load_workbook(XLSX, data_only=True)
ws      = wb['Roteiros']   # plural!
merge_map = build_merge_map(ws)
act = make_activity_fn(ws, merge_map, first_col=4)

DATES     = ["2019-10-25","2019-10-26","2019-10-27","2019-10-28","2019-10-29","2019-10-30"]
SUMMARIES = ["Ida","Bento Gon" + chr(231) + "alves","Bento Gon" + chr(231) + "alves",
             "Serra Geral","Aparados da Serra","Volta"]

DAYS_DEF = [
    (3, 0, [(7,'a01','Transporte'),(9,'a02','Transporte'),(10,'a03','Hospedagem')]),
    (4, 1, [(4,'a04','Passeio'),(6,'a05','Refeicao'),(7,'a06','Passeio'),(10,'a07','Hospedagem')]),
    (5, 2, [(4,'a08','Passeio'),(6,'a09','Refeicao'),(7,'a10','Transporte'),(10,'a11','Hospedagem')]),
    (6, 3, [(4,'a12','Transporte'),(5,'a13','Passeio'),(7,'a14','Passeio'),(9,'a15','Transporte'),(10,'a16','Hospedagem')]),
    (7, 4, [(4,'a17','Transporte'),(5,'a18','Passeio'),(7,'a19','Passeio'),(9,'a20','Transporte'),(10,'a21','Hospedagem')]),
    (8, 5, [(4,'a22','Transporte'),(6,'a23','Refeicao'),(7,'a24','Transporte')]),
]
...
def parse_gastos_brl(ws_g, ws_g_data, pfx, pi, ti, ppli, qi, pagi):
    ...
    for rr, rd in zip(ws_g.iter_rows(min_row=2), ws_g_data.iter_rows(min_row=2)):
        ...
        obs_parts = [p for p in [obs1,obs2,obs3,obs4] if p and p not in ('-','')]
        notes = ' . '.join(obs_parts) or None
        if etype == 'Hotel': etype = 'Hospedagem'
        if etype in ('Atividades','Passeios e Ingressos'): etype = 'Passeios'
        if not etype:
            tl = title.lower()
            if any(w in tl for w in ['hotel','pousada','cabana','laghetto']): etype='Hospedagem'
            elif any(w in tl for w in ['v' + chr(244) + 'o','aluguel','traslado']): etype='Transporte'
            elif any(w in tl for w in ['ingresso','trilha','parque']): etype='Passeios'
...
# pi=7=H, ti=8=I, ppli=9=J, qi=10=K, pagi=12=M
...
trip = {
    "schemaVersion":1,"id":PFX,"title":"2019-10 Serra Ga" + chr(250) + "cha",
    "startDate":"2019-10-25","endDate":"2019-10-30",
