...
FOLDER = r'G:\Meu Drive\Viagens\2017-11 Gramado'
XLSX   = FOLDER + r'\Viagem Gramado.xlsx'
PFX    = 'gramado-2017'

wb      = openpyxl.load_workbook(XLSX)
wb_data = openpyxl.load_workbook(XLSX, data_only=True)

# Col A(0)=date, Col C(2)=summary, activities D-J (0-idx 3-9), hosp_col=J(9), skip K(10)=shows
itinerary = parse_xlsx_roteiro(wb_data['Roteiro'],
    date_col=0, sum_col=2, act_cols=range(3, 11), first_col=3, data_start=3,
    pfx=PFX, hosp_col=9, skip_cols={10})
...
trip = build_trip(PFX, "2017-11 Gramado",
    start, end, 5, maps_url, itinerary, expenses, links, attachments)
